...
if __name__=="__main__":
    ...
    close_value = [0]*len(syms)
    
    for i,sym in enumerate(syms.index):
        start = datetime.datetime(2017,1,13)
        end = datetime.date.today()
        print sym
        try:
            stock = pdr.DataReader(sym,"google",start,end)
            close_value[i] = stock['Close'][-1]
        except (AttributeError,RemoteDataError): #No data available for stock or google has no record of stock
            close_value[i] = 'N/A'

    syms['Close'] = close_value
